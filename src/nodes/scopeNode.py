...
from src.states.scopeState import AgentState, ClarifyWithUser, ResearchQuestion
...
def clarify_with_user(state: AgentState) -> Command[Literal["write_research_brief", "__end__"]]:
    ...
    if not messages:
        ...
        return Command(
            goto=END,
            update={"messages": [AIMessage(content="No input provided. Please provide a research query.")]}
        )

    prompt = clarification_with_user_instrucitons.format(
    ...
    if response is None:
        ...
        return Command(
            goto=END,
            update={"messages": [AIMessage(content="Failed to process request. Please clarify your query.")]}
        )
